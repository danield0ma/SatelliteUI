...
export default function Commands() {
  ...
  const handleSendCommand = () => {
    // In a real application, this would send the command to the satellite
        ...
        <div className="md:col-span-2">
          <Card className="h-full">
            <CardContent className="p-6">
              ...
              <p className="text-muted-foreground mb-6">{selectedCommand.description}</p>
              ...
              <div className="space-y-6">
                    ...
                    {param.type === "number" && (
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>{param.min}</span>
                          <span>{paramValues[param.name] ?? param.default}</span>
                          <span>{param.max}</span>
                        </div>
                        <Slider
                          id={param.name}
                ...
                <div className="flex justify-end space-x-4 mt-8">
                  <Button variant="outline" onClick={() => setParamValues({})}>
                    {t("commands.reset")}
                  </Button>
                  <Button onClick={handleSendCommand}>{t("commands.send")}</Button>
                </div>
              </div>
            </CardContent>
          </Card>
